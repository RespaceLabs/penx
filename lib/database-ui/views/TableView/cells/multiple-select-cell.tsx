  ...
  return (
    <div>
      <Input
        ...
        size="sm"
        className="px-2 border-b rounded-none"
        autoFocus
        {...getInputProps({
          onChange: (e: any) => {
      ...
      <div>
        {!items.length && (
          <div className="flex items-center justify-between p-1 text-sm text-foreground/40">
            No options
          </div>
        )}
          ...
          {items.map((item, index) => (
            ...
            >
              <div className="flex items-center gap-2">
                {item.id === 'CREATE' && (
                  <div className="text-xs text-foreground/40">Create</div>
                )}
